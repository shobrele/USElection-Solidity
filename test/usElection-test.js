...
describe("USElection", function () {   
    ...
    it("Should throw when try to submit with equal state results", async function () {
        const stateResults = ["Texas",1200,1200,33];
        expect(usElection.submitStateResult(stateResults)).to.be.revertedWith('There cannot be a tie');
    });

    it("Should throw when try to submit with < 1 seat number", async function () {
        const stateResults = ["Dallas",1200,200,0];
        expect(usElection.submitStateResult(stateResults)).to.be.revertedWith('tates must have at least 1 seat');
    });
    ...
    it("Should throw when try to end election when they previously got ended already", async function () {
